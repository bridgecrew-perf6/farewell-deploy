...
def authenticate_user_landlord(username: str, password: str, db): 
    landlord = db.query(models.Landlords).filter(models.Landlords.username == username)\
        .first()

    if not landlord:
        return False

    if not verify_password(password, landlord.hashed_password):
        return False
    return landlord 
...
async def login_for_access_token(response: Response, form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    user = authenticate_user_landlord(form_data.username, form_data.password, db)
    if not user:
        return False
...
async def login(request:Request, db:Session = Depends(get_db)):
    try:
        ...
        response = RedirectResponse(url = "/addlisting", status_code=status.HTTP_302_FOUND)
...
async def logout(request: Request):
    ...
    response = templates.TemplateResponse("landlord_login.html", {"request": request, "msg": msg})
